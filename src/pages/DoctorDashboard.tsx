            ...
            <div className="max-h-[calc(100vh-250px)] overflow-y-auto">
              <ul className="divide-y divide-gray-200">
                {filteredPatients.map(patient => (
                  <li 
                    key={patient._id}
                    className={`py-3 px-2 cursor-pointer hover:bg-gray-50 ${selectedPatient?._id === patient._id ? 'bg-teal-50 border-l-4 border-teal-500' : ''}`}
        ...
        {selectedPatient ? (
          <div>
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h3 className="text-lg font-semibold mb-4 text-teal-700 border-b pb-2">Personal Information</h3>
                ...
                <div>
                  <h3 className="text-lg font-semibold mb-4 text-teal-700 border-b pb-2">Medical Information</h3>
                  
                  <div className="mb-4">
                    <p className="text-sm text-gray-500">Chief Complaint</p>
                    <p className="font-medium">{selectedPatient.chiefComplaint || 'None specified'}</p>
                  </div>
                  
                  <div className="mb-4">
                    <p className="text-sm text-gray-500">Medical History</p>
                    <p className="font-medium">{selectedPatient.medicalHistory || 'None specified'}</p>
                  </div>
                  
                  <div className="mb-4">
                    <p className="text-sm text-gray-500">Current Medications</p>
                    <p className="font-medium">{selectedPatient.currentMedications || 'None specified'}</p>
                  </div>
                  
                  <div className="mb-4">
                    <p className="text-sm text-gray-500">Allergies</p>
                    <p className="font-medium">{selectedPatient.allergies || 'None specified'}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col items-center justify-center h-full py-12 px-4 text-center">
            ...
            <p className="text-gray-500 max-w-md">
              Select a patient from the list to view their detailed information and medical history.
            </p>
          </div>
        )}
      </div>
